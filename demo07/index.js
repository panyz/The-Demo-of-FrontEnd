(function () {
    ...
    $gallery.addEventListener('click',function (e) {
        ...
        if (classList.contains(css_preview)) {
            ...
            //在预览模式下，所有图片排成一排，相册的宽度为每一个项目的宽度总和
            $gallery.style.width = 100 * items.length + "vw";
        }
    });
    ...
    $gallery.addEventListener('touchend',function (e) {
        if (isTouchstart) {
            //移动图片时需要动画，动画采用CSS3的transition实现
            ...
            if (Math.abs(dx) > width/2) {
                //处理临界值
                if(currentTranX <= 0 && currentTranX > -width * items.length){
                    //如果手指向右滑动
                    if (dx > 0) {
                        //如果图片不是显示第一张
                        if(currentTranX < 0){
                    ...
                    } else if(currentTranX > -width *(items.length -1)){
                        currentTranX = currentTranX - width;
                    }
                }
            }
